    ...
    <main className="max-w-5xl mx-auto px-4 py-8 space-y-8">
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 px-4">
          <div className="rounded-2xl p-6 w-full max-w-sm shadow-2xl" style={{ background: '#ffffff', border: '1px solid #e2e8f0' }}>
              ...
              <Card key={g.id} className="relative group flex flex-col">
                ...
                <div className="flex flex-col items-center text-center mb-3">
                  ...
                  <input
                    ...
                    className="text-base font-semibold text-gray-900 text-center border-0 bg-transparent focus:outline-none focus:bg-transparent rounded px-1 w-full"
                  />
                </div>
                    ...
                    {[...(g.contributions || [])].reverse().map(c => (
                      <div key={c.id} className="flex items-center justify-between group/entry">
                        <div className="flex-1 min-w-0">
                          ...
                          {c.note && <span className="text-xs text-gray-500 ml-1">— {c.note}</span>}
                          <span className="text-xs text-gray-400 ml-1">{c.date}</span>
                        </div>
                        <button
                          onClick={() => deleteContribution(g.id, c.id)}
                          className="text-gray-400 hover:text-red-400 opacity-0 group-hover/entry:opacity-100 transition-opacity text-xs ml-1 flex-shrink-0"
                        >✕</button>
                      </div>
                    ))}
                  </div>
